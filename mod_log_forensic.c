...
module AP_MODULE_DECLARE_DATA log_forensic_module2;
...
{
    fcfg *cfg = ap_get_module_config(s->module_config, &log_forensic_module2);
    ...
    if (*cfg->logname == '|')
    {
        ...
        if (pl == NULL)
        {
            ap_log_error(APLOG_MARK, APLOG_ERR, 0, s, APLOGNO(00650) "couldn't spawn forensic log pipe %s", cfg->logname);
            return 0;
        }
    ...
    else
    {
        ...
        {
            ap_log_error(APLOG_MARK, APLOG_ERR, rv, s, APLOGNO(00651) "could not open forensic log file %s.", fname);
            return 0;
        }
    }
...
static int log_before(request_rec *r)
{
    ...
    ap_set_module_config(r->request_config, &log_forensic_module2, (char *)id);
